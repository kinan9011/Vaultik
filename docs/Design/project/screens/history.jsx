/* Run History — searchable log of every operation */
const {
  IconHistory, IconCheck, IconAlert, IconX, IconRefresh, IconClock,
  IconChevronRight, IconChevronDown, IconFilter, IconSearch,
  IconPlay, IconRestore, IconShield, IconDownload, IconTrash,
} = window.Icons;
const { Sidebar, TopBar } = window.Shell;

const RUNS = [
  {
    status: "running", profile: "Home Documents", op: "backup", trigger: "manual",
    started: "Today, 14:26", duration: "Running…", expanded: true,
    details: { files: "14,523 / 21,890", added: "318 MB", changed: "642", new: "84" },
  },
  { status: "success", profile: "Photos Library", op: "backup", trigger: "scheduled",
    started: "Today, 12:00", duration: "8m 14s",
    details: { files: "412,008", added: "12 MB", changed: "4", new: "12" } },
  { status: "success", profile: "Production DB Server", op: "check", trigger: "scheduled",
    started: "Today, 11:30", duration: "2m 41s" },
  { status: "warn", profile: "Workstation Projects", op: "backup", trigger: "scheduled",
    started: "Today, 09:04", duration: "12m 06s",
    note: "3 files skipped (permission denied)" },
  { status: "success", profile: "Production DB Server", op: "backup", trigger: "scheduled",
    started: "Today, 09:00", duration: "1m 38s" },
  { status: "success", profile: "Home Documents", op: "restore", trigger: "manual",
    started: "Yesterday, 17:42", duration: "3m 22s",
    note: "Restored 142 files to /tmp/restore-may-18" },
  { status: "fail", profile: "Workstation Projects", op: "backup", trigger: "scheduled",
    started: "Yesterday, 18:00", duration: "0m 04s",
    note: "Failed: repository is locked (held by PID 4128 on db-01)" },
  { status: "success", profile: "Production DB Server", op: "forget", trigger: "scheduled",
    started: "Yesterday, 03:00", duration: "0m 12s",
    note: "Removed 2 snapshots · pruned 384 MB" },
  { status: "success", profile: "Photos Library", op: "backup", trigger: "scheduled",
    started: "May 17, 03:00", duration: "11m 02s" },
];
    ...
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{
          ...
          fontSize: 11, fontWeight: 500, fontVariantCaps: "all-small-caps",
          letterSpacing: "0.04em", textTransform: "uppercase",
          minWidth: 78, justifyContent: "center",
        }}>
          <s.icon size={10} style={r.status === "running" ? { animation: "spin 1.6s linear infinite" } : null} />
          {s.label}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
          ...
          <span className="badge" style={{ fontSize: 10.5 }}>{r.trigger}</span>
        </div>

        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11.5, color: "var(--text-2)" }}>{r.duration}</span>
          <span style={{ fontSize: 11, color: "var(--text-4)", marginTop: 2 }}>{r.started}</span>
        </div>
      ...
      {r.expanded && r.details && (
        <div style={{
          ...
          <div style={{ gridColumn: "1 / -1", marginTop: 4, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{
              fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)",
              padding: "3px 8px", borderRadius: 4, background: "var(--surface-2)",
            }}>snapshot 8a2f7c1d</span>
            <span style={{ flex: 1 }} />
            <button className="btn btn-sm btn-ghost"><IconDownload size={11} /> Export log</button>
          </div>
        </div>
      )}
      ...
      {!r.expanded && r.note && (
        <div style={{
          ...
          borderTop: "1px solid var(--border)",
          fontSize: 11.5,
          color: r.status === "fail" ? "var(--danger)" : r.status === "warn" ? "var(--warn)" : "var(--text-3)",
          fontFamily: "var(--font-mono)",
          background: r.status === "fail" ? "rgba(248,113,113,0.04)"
                    : r.status === "warn" ? "rgba(251,191,36,0.04)"
                    : "var(--bg-2)",
        }}>
          {r.note}
        </div>
  ...
  <div className="v-app">
    ...
    <div className="v-main">
      <TopBar
        ...
        actions={
          <>
            <button className="btn"><IconDownload size={12} /> Export</button>
          </>
        }
      />
        ...
        <div style={{ maxWidth: 1100 }}>
          ...
          <div className="card" style={{ marginBottom: 18, padding: "12px 14px",
            ...
            <div style={{ position: "relative", flex: 1, minWidth: 220 }}>
              ...
              <input className="input" placeholder="Search by profile, operation, error message…" style={{ paddingLeft: 28 }} />
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            {RUNS.slice(0, 4).map((r, i) => <RunRow key={i} r={r} />)}
          </div>

          <DaySep label="Yesterday, May 18" count={3} />
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            {RUNS.slice(4, 7).map((r, i) => <RunRow key={i} r={r} />)}
          </div>

          <DaySep label="Saturday, May 17" count={2} />
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {RUNS.slice(7).map((r, i) => <RunRow key={i} r={r} />)}
          </div>
        </div>
      </div>
    </div>
...
window.Screens.RunHistory = RunHistory;
